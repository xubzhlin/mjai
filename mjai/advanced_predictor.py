...
from .advanced_model import AdvancedMahjongModel, AdvancedMahjongNet
...
class AdvancedPredictionConfig:
    ...
    use_grp: bool = True  # 是否使用GRP
    grp_hidden_dim: int = 128  # GRP隐藏维度
    grp_layers: int = 2  # GRP层数


class AdvancedActionPredictor:
    """高级动作预测器 - 支持ResNet + Dueling DQN + GRP架构"""
    
    def __init__(self, 
                 model: AdvancedMahjongModel,
                 feature_processor: FeatureProcessor,
                 action_encoder: ActionEncoder,
                 config: Optional[AdvancedPredictionConfig] = None):
        """
        初始化高级动作预测器
        ...
        self.config = config or AdvancedPredictionConfig()
    ...
    def predict_action(self, game_state: Dict) -> Tuple[int, float, Dict]:
        ...
        # 预测动作概率和Q值
        if self.config.use_grp and self.grp_hidden is not None:
            action_probs, q_value, self.grp_hidden = self.model.predict(
                normalized_features, self.grp_hidden
            )
        else:
            action_probs, q_value, self.grp_hidden = self.model.predict(normalized_features)
        ...
        if random.random() < self.epsilon:
            ...
            # 利用：根据Q值选择动作
            action_idx = valid_action_indices[np.argmax(valid_q_values)]
        ...
        # 更新GRP使用统计
        if self.config.use_grp:
            self.stats['grp_usage'] += 1
        
        # 计算平均Q值
        self.stats['avg_q_value'] = np.mean(valid_q_values)
        ...
        prediction_info = {
            ...
            'top_actions': self._get_top_actions(valid_action_indices, valid_probs, valid_q_values),
            'grp_hidden_used': self.config.use_grp,
            'game_state': {
    ...
    def predict_with_beam_search(self, game_state: Dict) -> Tuple[int, float, Dict]:
        ...
        # 预测当前状态Q值
        if self.config.use_grp and self.grp_hidden is not None:
            _, current_q_value, self.grp_hidden = self.model.predict(normalized_features, self.grp_hidden)
        else:
            _, current_q_value, self.grp_hidden = self.model.predict(normalized_features)
        
        # 束搜索
        beam = [(action_idx, current_q_value, [action_idx]) for action_idx in valid_actions]
        
        for depth in range(self.config.max_search_depth):
            new_beam = []
            
            for action_idx, q_value, action_sequence in beam:
                # 模拟动作后的状态
                ...
                if self.config.use_grp and self.grp_hidden is not None:
                    _, new_q_value, self.grp_hidden = self.model.predict(normalized_new_features, self.grp_hidden)
                else:
                    _, new_q_value, self.grp_hidden = self.model.predict(normalized_new_features)
                
                # 计算累计Q值
                cumulative_q_value = q_value + new_q_value * (self.model.gamma ** (depth + 1))
                
                new_beam.append((action_idx, cumulative_q_value, action_sequence + [action_idx]))
    ...
    def _get_top_actions(self, valid_actions: List[int], valid_probs: np.ndarray, valid_q_values: List[float]) -> List[Dict]:
        """获取Top-K动作"""
        if len(valid_actions) <= self.config.top_k:
            return [
                ...
                for action_idx, prob, q_value in zip(valid_actions, valid_probs, valid_q_values)
            ]
        
        # 根据Q值排序获取Top-K
        top_indices = np.argsort(valid_q_values)[-self.config.top_k:][::-1]
        return [
            {
                'action': valid_actions[i], 
                'prob': valid_probs[i],
                'q_value': valid_q_values[i]
            }
...
class AdvancedBatchPredictor:
    ...
    def predict_batch(self, game_states: List[Dict]) -> List[Tuple[int, float, Dict]]:
        ...
        with torch.no_grad():
            # 使用高级模型的Dueling DQN
            q_values_batch = self.model.model.dueling_dqn(self.model.model.resnet(features_tensor))
        
        results = []
        for i, (state, q_values) in enumerate(zip(game_states, q_values_batch)):
            # 获取合法动作
            ...
            for action_idx in valid_actions:
                if action_idx < len(q_values):
                    valid_probs.append(1.0 / len(valid_actions))  # 简化的概率
                    valid_action_indices.append(action_idx)
                    valid_q_values.append(q_values[action_idx].item())
...
def create_advanced_predictor(model_path: str, 
                            feature_processor: FeatureProcessor,
                            action_encoder: ActionEncoder,
                            config: Optional[AdvancedPredictionConfig] = None) -> AdvancedActionPredictor:
    """
    从模型路径创建高级预测器
    ...
    """
    # 加载模型
    model = AdvancedMahjongModel.load(model_path)
